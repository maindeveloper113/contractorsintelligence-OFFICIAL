...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *backCancelItem = [[UIBarButtonItem alloc] init];
    backCancelItem.title = NSLocalizedString(@"Use original", nil);
    [backCancelItem setTarget:self];
    [backCancelItem setAction:@selector(cancel:)];
    
    self.navigationItem.leftBarButtonItem = backCancelItem;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    CGFloat width = 600.0f;//CGRectGetWidth(cropRect);
...
//    CGRect cropRect1;
//    cropRect1.size = CGSizeMake(599.0f, 798.0f);
//    self.cropView.imageCropRect = cropRect1;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        cropRect.size = CGSizeMake(width, width * ratio);
//        cropRect.size = CGSizeMake(599.0f, 798.0f);
        self.cropView.cropRect = cropRect;
